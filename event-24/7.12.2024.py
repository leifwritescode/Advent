...
def validate_equation(target, operands):
    ...
    return validate_equation(target / val, operands[:-1]) or validate_equation(float(target - val), operands[:-1])
...
def part_one():
    ...
    return int(sum([equation[0] if validate_equation(equation[0], equation[1]) else 0 for equation in equations]))
