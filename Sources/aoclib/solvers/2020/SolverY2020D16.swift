...
class SolverY2020D16 : Solvable {
    ...
    required init(withLog log: Log, andInput input: String) {
        ...
        ranges = sections.removeFirst()
            .reduce([String:[Int]]()) { dict, line in
            var dict = dict
            let groups = line.groups(for: #"([\w\s]+):\s(\d+)-(\d+)\sor\s(\d+)-(\d+)"#).first!
            dict[groups[0]] = Array(Int(groups[1])!...Int(groups[2])!) + Array(Int(groups[3])!...Int(groups[4])!)
            return dict
        }
    ...
    func doPart2(withLog log: Log) {
        // Filter out tickets that have invalid fields
        log.info(theMessage: "Filtering invalid tickets...")
        let candidates = tickets.filter { ticket in
            ticket.filter { value in ranges.allSatisfy { _, v in !v.contains(value) }}.count == 0
        }
        log.info(theMessage: "Done!")

        // Track the assigned fields in order of assignment.
        ...
        outer: while assigned.count != ranges.count {
            ...
            inner: for i in range {
                // Map the column to a single dimension and iterate until we exhaust it or break out.
                let col = candidates.reduce([Int]()) { r, a in r + [a[i]] }
                var iter = col.enumerated().makeIterator()
                var curr = iter.next()
                while curr != nil {
                    // Find all potential fields for this row, and intersect with the unassigned rows.
